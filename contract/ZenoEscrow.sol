...
interface IZenoVulnerabilityEscrow {
...
/**
 * @title ZenoEscrow
 * @notice Manages staking and incentive payments for the Zeno vulnerability platform.
 *         Incentive #1 (bounty) is only releasable when ZenoVulnerability status = Verified (1).
 *         Incentive #2 (fix)   is only releasable when ZenoVulnerability status = FixVerified (5).
...
contract ZenoEscrow {
    ...
    address public zenoVulnerabilityContract;
    ...
    constructor(address _zenoVulnerabilityContract, uint256 _requiredStake) {
        admin = msg.sender;
        zenoVulnerabilityContract = _zenoVulnerabilityContract;
     ...
     *         Call this before submitVulnerability() on ZenoVulnerability.
    ...
    function setBountyAmount(uint256 submissionId, uint256 amount) external onlyAdmin {
        IZenoVulnerabilityEscrow vuln = IZenoVulnerabilityEscrow(zenoVulnerabilityContract);
    ...
    /**
     * @notice Auto-slash triggered by ZenoVulnerability when fraud is detected (Phase 3).
    ...
    function fraudSlash(uint256 submissionId) external nonReentrant {
        require(msg.sender == zenoVulnerabilityContract, "Only ZenoVulnerability can call fraudSlash");
